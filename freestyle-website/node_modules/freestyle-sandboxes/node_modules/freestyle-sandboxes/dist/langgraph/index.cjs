...
function nodeDataJson(node) {
  if (isRunnableInterface(node.data)) {
    ...
    return {
      type: "schema",
      data: { ...index.zodToJsonSchema(node.data.schema), title: node.data.name }
    };
  }
}
class Graph {
...
function convertRunnableToTool(runnable, fields) {
  ...
  if (fields.schema.constructor === zod.z.ZodString) {
...
class DynamicStructuredTool extends StructuredTool {
  ...
  constructor(fields) {
    ...
    this.schema = isZodSchema(fields.schema) ? fields.schema : zod.z.object({}).passthrough();
  }
  /**
   * @deprecated Use .invoke() instead. Will be removed in 0.3.0.
   */
...
const executeTool = (config) => {
  ...
  return new DynamicStructuredTool({
    ...
    schema: index.executeCodeSchema,
    func: async ({ script }) => {
