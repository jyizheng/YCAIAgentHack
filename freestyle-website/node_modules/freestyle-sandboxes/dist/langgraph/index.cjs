...
function validate$2(instance, schema, draft = "2019-09", lookup = dereference(schema), shortCircuit = true, recursiveAnchor = null, instanceLocation = "#", schemaLocation = "#", evaluated = /* @__PURE__ */ Object.create(null)) {
  if (schema === true) {
  ...
  if ($recursiveRef === "#") {
    ...
    const result = validate$2(instance, recursiveAnchor === null ? schema : recursiveAnchor, draft, lookup, shortCircuit, refSchema, instanceLocation, keywordLocation, evaluated);
    if (!result.valid) {
      errors.push({
        instanceLocation,
...
- ${Object.keys(lookup).join("\n- ")}`;
    ...
    const result = validate$2(instance, refSchema, draft, lookup, shortCircuit, recursiveAnchor, instanceLocation, keywordLocation, evaluated);
    if (!result.valid) {
      errors.push({
        instanceLocation,
        keyword: "$ref",
  ...
  if ($not !== void 0) {
    ...
    const result = validate$2(instance, $not, draft, lookup, shortCircuit, recursiveAnchor, instanceLocation, keywordLocation);
    if (result.valid) {
      errors.push({
        instanceLocation,
        keyword: "not",
  ...
  if ($anyOf !== void 0) {
    ...
    for (let i = 0; i < $anyOf.length; i++) {
      ...
      const result = validate$2(instance, subSchema, draft, lookup, shortCircuit, $recursiveAnchor === true ? recursiveAnchor : null, instanceLocation, `${keywordLocation}/${i}`, subEvaluated);
  ...
  if ($if !== void 0) {
    ...
    const conditionResult = validate$2(instance, $if, draft, lookup, shortCircuit, recursiveAnchor, instanceLocation, keywordLocation, evaluated).valid;
    if (conditionResult) {
      if ($then !== void 0) {
        const thenResult = validate$2(instance, $then, draft, lookup, shortCircuit, recursiveAnchor, instanceLocation, `${schemaLocation}/then`, evaluated);
        if (!thenResult.valid) {
          errors.push({
            instanceLocation,
            keyword: "if",
      ...
      const elseResult = validate$2(instance, $else, draft, lookup, shortCircuit, recursiveAnchor, instanceLocation, `${schemaLocation}/else`, evaluated);
      if (!elseResult.valid) {
        errors.push({
          instanceLocation,
          keyword: "if",
  ...
  if (instanceType === "object") {
    ...
    if ($propertyNames !== void 0) {
      ...
      for (const key in instance) {
        ...
        const result = validate$2(key, $propertyNames, draft, lookup, shortCircuit, recursiveAnchor, subInstancePointer, keywordLocation);
        if (!result.valid) {
          errors.push({
            instanceLocation,
    ...
    if ($dependentSchemas !== void 0) {
      for (const key in $dependentSchemas) {
        ...
        if (key in instance) {
          const result = validate$2(instance, $dependentSchemas[key], draft, lookup, shortCircuit, recursiveAnchor, instanceLocation, `${keywordLocation}/${encodePointer(key)}`, evaluated);
          if (!result.valid) {
            errors.push({
              instanceLocation,
      ...
      for (const key in $dependencies) {
        if (key in instance) {
          ...
          if (Array.isArray(propsOrSchema)) {
            ...
            const result = validate$2(instance, propsOrSchema, draft, lookup, shortCircuit, recursiveAnchor, instanceLocation, `${keywordLocation}/${encodePointer(key)}`);
            if (!result.valid) {
              errors.push({
                instanceLocation,
                keyword: "dependencies",
    ...
    if ($properties !== void 0) {
      ...
      for (const key in $properties) {
        ...
        const subInstancePointer = `${instanceLocation}/${encodePointer(key)}`;
        const result = validate$2(instance[key], $properties[key], draft, lookup, shortCircuit, recursiveAnchor, subInstancePointer, `${keywordLocation}/${encodePointer(key)}`);
        if (result.valid) {
    ...
    if (!stop && $patternProperties !== void 0) {
      ...
      for (const pattern in $patternProperties) {
        ...
        for (const key in instance) {
          ...
          const subInstancePointer = `${instanceLocation}/${encodePointer(key)}`;
          const result = validate$2(instance[key], subSchema, draft, lookup, shortCircuit, recursiveAnchor, subInstancePointer, `${keywordLocation}/${encodePointer(pattern)}`);
          if (result.valid) {
            evaluated[key] = thisEvaluated[key] = true;
          } else {
            stop = shortCircuit;
    ...
    if (!stop && $additionalProperties !== void 0) {
      ...
      for (const key in instance) {
        ...
        const subInstancePointer = `${instanceLocation}/${encodePointer(key)}`;
        const result = validate$2(instance[key], $additionalProperties, draft, lookup, shortCircuit, recursiveAnchor, subInstancePointer, keywordLocation);
        if (result.valid) {
          evaluated[key] = true;
        } else {
          stop = shortCircuit;
      ...
      for (const key in instance) {
        if (!evaluated[key]) {
          const subInstancePointer = `${instanceLocation}/${encodePointer(key)}`;
          const result = validate$2(instance[key], $unevaluatedProperties, draft, lookup, shortCircuit, recursiveAnchor, subInstancePointer, keywordLocation);
          if (result.valid) {
            evaluated[key] = true;
          } else {
            errors.push({
    ...
    if ($prefixItems !== void 0) {
      ...
      for (; i < length2; i++) {
        const result = validate$2(instance[i], $prefixItems[i], draft, lookup, shortCircuit, recursiveAnchor, `${instanceLocation}/${i}`, `${keywordLocation}/${i}`);
        evaluated[i] = true;
        if (!result.valid) {
          stop = shortCircuit;
          errors.push({
    ...
    if ($items !== void 0) {
      ...
      if (Array.isArray($items)) {
        ...
        for (; i < length2; i++) {
          const result = validate$2(instance[i], $items[i], draft, lookup, shortCircuit, recursiveAnchor, `${instanceLocation}/${i}`, `${keywordLocation}/${i}`);
          evaluated[i] = true;
          if (!result.valid) {
            stop = shortCircuit;
            errors.push({
        ...
        for (; i < length; i++) {
          const result = validate$2(instance[i], $items, draft, lookup, shortCircuit, recursiveAnchor, `${instanceLocation}/${i}`, keywordLocation);
          evaluated[i] = true;
          if (!result.valid) {
            stop = shortCircuit;
            errors.push({
      ...
      if (!stop && $additionalItems !== void 0) {
        ...
        for (; i < length; i++) {
          const result = validate$2(instance[i], $additionalItems, draft, lookup, shortCircuit, recursiveAnchor, `${instanceLocation}/${i}`, keywordLocation2);
          evaluated[i] = true;
          if (!result.valid) {
            stop = shortCircuit;
            errors.push({
    ...
    if ($contains !== void 0) {
      if (length === 0 && $minContains === void 0) {
        ...
        for (let j = 0; j < length; j++) {
          const result = validate$2(instance[j], $contains, draft, lookup, shortCircuit, recursiveAnchor, `${instanceLocation}/${j}`, keywordLocation);
          if (result.valid) {
            evaluated[j] = true;
            contained++;
          } else {
    ...
    if (!stop && $unevaluatedItems !== void 0) {
      ...
      for (i; i < length; i++) {
        ...
        const result = validate$2(instance[i], $unevaluatedItems, draft, lookup, shortCircuit, recursiveAnchor, `${instanceLocation}/${i}`, keywordLocation);
        evaluated[i] = true;
        if (!result.valid) {
          errors.push({
            instanceLocation,
...
var snakeCase = /*@__PURE__*/index.getDefaultExportFromCjs(decamelizeExports);
...
var styles = /*@__PURE__*/index.getDefaultExportFromCjs(ansiStylesExports);
...
var pRetry = /*@__PURE__*/index.getDefaultExportFromCjs(pRetryExports);
...
var PQueueMod = /*@__PURE__*/index.getDefaultExportFromCjs(distExports);
...
var base64 = /*@__PURE__*/index.getDefaultExportFromCjs(base64JsExports);
...
async function interopParseAsync(schema, input) {
  if (isZodSchemaV4(schema)) {
    return core.parse(schema, input);
  }
...
function getSchemaDescription(schema) {
  if (isZodSchemaV4(schema)) {
    return core.globalRegistry.get(schema)?.description;
  }
  if (isZodSchemaV3(schema)) {
    ...
    if (recursive) {
      for (const [key, keySchema] of Object.entries(schema._zod.def.shape)) {
        if (isZodObjectV4(keySchema)) {
          ...
          outputShape[key] = core.clone(keySchema, {
            ...keySchema._zod.def,
        ...
        const meta2 = core.globalRegistry.get(keySchema);
        if (meta2)
          core.globalRegistry.add(outputShape[key], meta2);
      }
    }
    const modifiedSchema = core.clone(schema, {
      ...schema._zod.def,
      shape: outputShape,
      catchall: core._never(core.$ZodNever)
    });
    const meta = core.globalRegistry.get(schema);
    if (meta)
      core.globalRegistry.add(modifiedSchema, meta);
  ...
  if (isZodSchemaV4(schema)) {
    ...
    if (recursive) {
      if (isZodObjectV4(outputSchema)) {
        ...
        outputSchema = core.clone(outputSchema, {
    ...
    const meta = core.globalRegistry.get(schema);
    if (meta)
      core.globalRegistry.add(outputSchema, meta);
    return outputSchema;
  }
  throw new Error("Schema must be an instance of z3.ZodType or z4.$ZodType");
}
...
function toJsonSchema(schema) {
  if (isZodSchemaV4(schema)) {
    ...
    if (isZodObjectV4(inputSchema)) {
      ...
      return core.toJSONSchema(strictSchema);
    } else {
      return core.toJSONSchema(schema);
    }
  }
  if (isZodSchemaV3(schema)) {
    return index.zodToJsonSchema(schema);
  }
  return schema;
}
...
function convertRunnableToTool(runnable, fields) {
  ...
  if (isSimpleStringZodSchema(fields.schema)) {
    return new RunnableToolLike({
      name,
      description,
      schema: v3.z.object({ input: v3.z.string() }).transform((input) => input.input),
      bound: runnable
    });
  }
  return new RunnableToolLike({
...
Details: ${e.message}`;
      ...
      const result2 = validate$2(inputForValidation, this.schema);
      if (!result2.valid) {
        let message = `Received tool input did not match expected schema`;
        if (this.verboseParsingErrors) {
          message = `${message}
...
class Tool extends StructuredTool {
  constructor(fields) {
    ...
    Object.defineProperty(this, "schema", {
      ...
      configurable: true,
      writable: true,
      value: v3.z.object({ input: v3.z.string().optional() }).transform((obj) => obj.input)
    });
  }
  /**
   * @deprecated Use .invoke() instead. Will be removed in 0.3.0.
...
const executeTool = (config) => {
  ...
  return tool(
    ...
    {
      name: "executeTool",
      description: index.executeCodeDescription(
